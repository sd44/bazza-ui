...
Before we continue, let's take a look at the most important definitions we'll need to know - `ColumnDataType` and `ColumnMeta`.
...
- The `transformOptionFn` function is used to convert the raw option value to a `ColumnOption` object. This is used when you don't provide a static options list via the `options` property, and instead want to infer the available options from the column data itself.
