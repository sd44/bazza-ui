...
Before we continue, let's take a look at the most important definitions we'll be using throughout this guide - `ColumnDataType` and `ColumnMeta`.
...
You set the `type` property of the `ColumnMeta` interface (explained below) to one of the following values:
...
The star of the show is the `ColumnMeta` interface, which defines the metadata for a column.
...
To keep this concise, I'll skip over the self-explanatory properties: `displayName`, `icon`, and `type`.

If you're dealing with `number` data, you can define a `max` property to set a "soft" maximum value for the range slider.

For `option` and `multiOption` columns:

- You can define the `options` property to provide a list of options ahead of time.
- The `transformFn` function is used to convert the column value to the native value for the column data type.
- The `transformOptionFn` function is used to convert the raw option value to a `ColumnOption` object. This is used when we omit a static options list via the `options` property, and instead infer the available options from the column data itself.
...
For the filter function, we provide one - it is conveniently called `filterFn` and takes a single required argument `type` which is the column data type (i.e. `ColumnDataType`).
...
- **Remote**: we need to fetch the options from a remote source *(Coming soon)*
